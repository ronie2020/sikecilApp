<nav 
    ...
    <div class="relative z-10 h-24 flex items-center px-6 border-b border-white/5 bg-slate-900">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-4 group w-full relative z-10">
            ...
            <div class="flex flex-col">
                ...
                <span class="text-[10px] text-emerald-400 font-medium tracking-wider uppercase">Portal Sekolah</span>
            </div>
        </a>
    </div>
    ...
    <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1 custom-scrollbar">
        @php
            ...
            // Hitung Notifikasi Chat (Guru)
            $unreadChat = 0;
            if($role === 'guru') {
                $unreadChat = \App\Models\BukuPenghubung::where('is_read', 0)
                                ->where('pengirim_id', '!=', $user->id) 
                                ->count();
            }

            // Hitung Notifikasi Chat (Siswa)
            $unreadSiswa = 0;
            if($role === 'siswa') {
                $unreadSiswa = \App\Models\BukuPenghubung::where('is_read', 0)
                                ->where('pengirim_id', '!=', $user->id)
            ...
            $menuStructure = [
                ...
                'admin' => [
                    ...
                    'Ekstrakurikuler' => [
                        ['name' => 'Manajemen Ekskul', 'route' => 'admin.ekskul.index', 'active' => 'admin.ekskul.*', 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
                        ['name' => 'Info Kegiatan (Publik)', 'route' => 'ekskul', 'icon' => 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ],
                    'Administrasi' => [
                ...
                'guru' => [
                    ...
                    'Ekstrakurikuler' => [
                        ['name' => 'Absensi Ekskul', 'route' => 'admin.ekskul.index', 'active' => 'admin.ekskul.*', 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01'],
                        ['name' => 'Info Kegiatan', 'route' => 'ekskul', 'icon' => 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ],
                    'Absensi & Karakter' => [
                    ...
                    'Akademik' => [
                        ...
                        ['name' => 'Buku Penghubung', 'route' => 'penghubung.index', 'active' => 'penghubung.*', 'icon' => 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z', 'badge' => $unreadChat],
                    ]
                ],
                ...
                'default' => [ 
                    'Menu Siswa' => [
                        ...
                        ['name' => 'Info Ekskul', 'route' => 'ekskul', 'icon' => 'M13 10V3L4 14h7v7l9-11h-7z'], 
                        ...
                        ['name' => 'Buku Penghubung', 'route' => 'penghubung.index', 'active' => 'penghubung.*', 'icon' => 'M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z', 'badge' => $unreadSiswa],
                    ]
                ]
            ];
